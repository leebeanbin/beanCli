...
| **웹 콘솔** | 게임보이 스타일 Next.js 앱 — 13개 페이지, 다크/라이트 테마, 한/영 전환 |
| **멀티라인 SQL 에디터** | 줄 번호, 커서 이동, 히스토리, psql 메타 커맨드 (`\dt`, `\d`, `\x`) |
...
| Query | `/query` | 결과 테이블이 있는 SQL 에디터 |
| Explore | `/explore` | 데이터 탐색기 — 행 CRUD, 상세 모달 |
| Schema | `/schema` | 테이블 구조 뷰어 + 쿼리 분석기 |
| Monitor | `/monitor` | 스트림 통계, SSE 실시간 업데이트 |
| Indexes | `/indexes` | 인덱스 목록, 생성, 삭제 |
| Audit | `/audit` | 불변 감사 로그 (카테고리 필터) |
| Recovery | `/recovery` | DLQ 실패 변경사항 재제출 |
| AI | `/ai` | 전용 AI 채팅 페이지 — `◈ AI` 플로팅 버튼으로 접근 |
| Changes | `/changes` | 변경 요청 목록 및 제출 |
| Approvals | `/approvals` | 승인 대기 큐 |
| Auth | `/auth` | Dev JWT 생성기 (HS256, localStorage 저장) |
| Connections | `/connections` | DB 연결 등록 & 테스트 |
...
| **요청 제한** | `@fastify/rate-limit` — 전역 60/분 · `/auth/login` 5/분 · `/connections/test` 10/분 |
| **로거** | Fastify pino가 `authorization`, `password`, `credential`, `secret` 난독화 |
