...
function printTable(bench: Bench): void {
  ...
  const row = (
    name: string, ops: string, avg: string,
    p50: string,  p75: string, p99: string,
  ...
  row(
    ' Benchmark', ' ops/s', ' avg', ' p50', ' p75', ' p99', ' min', ' max',
    true,
  );

  const sorted = [...bench.tasks].sort(
    (a, b) => safeHz(b.result) - safeHz(a.result),
  );
  ...
  for (const t of sorted) {
    ...
    if (!r || !('throughput' in r) || !('latency' in r)) {
      console.log(` ${t.name}  — (no result)`); continue;
    }
    const nr = r as unknown as V6Result;
    const opsColor = nr.throughput.mean >= 1_000_000 ? C.green : nr.throughput.mean >= 100_000 ? C.yellow : C.red;
    console.log(
...
async function runRedisBench(): Promise<void> {
  ...
  for (const n of KEY_COUNTS) {
    const naiveTask = bench.tasks.find(t => t.name.includes(`naïve N+1    (${String(n).padStart(3)}`));
    const pipeTask  = bench.tasks.find(t => t.name.includes(`two-pipeline (${String(n).padStart(3)}`));
...
async function main(): Promise<void> {
  ...
  console.log(`╚══════════════════════════════════════════════════════╝${C.reset}`);
  console.log(`  ${C.dim}Platform: ${process.platform}  Arch: ${process.arch}  CPU cores: ${(await import('node:os')).cpus().length}${C.reset}`);
