...
describe('AesEncryptor — throughput benchmarks', () => {
  ...
  for (const [label, plain] of Object.entries(payloads)) {
    it(`encrypt: ${label}`, async () => {
      const { opsPerSec, p99Us } = await runBench(
  ...
  it('encrypt + decrypt round-trip: 1KB payload', async () => {
    ...
    const encStats = await runBench(
      'AES-256-GCM encrypt 1KB',
      ITERS,
      () => encryptor.encrypt(plain, 'bench-key-001').then(r => { encrypted = r; }),
    );

    const decStats = await runBench(
  ...
  it('throughput: concurrent 10 encryptions', async () => {
    ...
    const { opsPerSec } = await runBench(
      ...
      () => Promise.all(Array.from({ length: BATCH }, () =>
        encryptor.encrypt(plain, 'bench-key-001'),
      )).then(() => undefined),
    );

    // Each concurrent batch counts as 1 op; total throughput = opsPerSec × BATCH
    const totalOpsPerSec = opsPerSec * BATCH;
    console.log(`  [INFO]   total concurrent throughput: ${totalOpsPerSec.toLocaleString()} encrypt-ops/s`);
...
describe('HmacHasher — throughput benchmarks', () => {
  ...
  it('throughput: concurrent 50 hashes', async () => {
    ...
    const { opsPerSec } = await runBench(
      `HMAC-SHA256 concurrent×${BATCH}`,
      ITERS_CONC,
      () => Promise.all(Array.from({ length: BATCH }, (_, i) =>
        hasher.hash('user', `usr-${i}`),
      )).then(() => undefined),
    );

    const totalOpsPerSec = opsPerSec * BATCH;
    console.log(`  [INFO]   total concurrent throughput: ${totalOpsPerSec.toLocaleString()} hash-ops/s`);
  ...
  it('AES vs HMAC relative performance', async () => {
    ...
    console.log(`  [INFO]   HMAC is ${speedup.toFixed(1)}× faster than AES-256-GCM (256B plaintext)`);
