...
describe('state routes', () => {
  ...
  describe('getStateById', () => {
    it('should return entity when found', async () => {
      const mockDb: IDbSession = {
        query: jest
          .fn()
          .mockResolvedValue({ rows: [{ entity_id_hash: 'abc', status: 'ACTIVE' }], rowCount: 1 }),
      };
  ...
  describe('updateStateField', () => {
    ...
    it('should reject invalid status values', async () => {
      ...
      await expect(
        updateStateField(db, 'state_orders', 'ord_001', 'status', 'BOGUS'),
    ...
    it('should reject invalid currency format', async () => {
      const db: IDbSession = { query: jest.fn() };
      await expect(
        updateStateField(db, 'state_orders', 'ord_001', 'currency_code', 'US'),
      ).rejects.toBeInstanceOf(StateValidationError);
    });
    ...
    it('should reject non-writable fields', async () => {
      const db: IDbSession = { query: jest.fn() };
      await expect(
        updateStateField(db, 'state_orders', 'ord_001', 'created_event_time_ms', 1),
  ...
  describe('insertStateRow', () => {
    ...
    it('should reject invalid payment method', async () => {
      const db: IDbSession = { query: jest.fn() };
      await expect(
        insertStateRow(db, 'state_payments', {
