...
-- CTE를 사용해 ACTIVE→PREVIOUS, PREVIOUS→RETIRED을
-- 원자적으로 처리하여 경합 조건 방지
-- -----------------------------------------------
...
RETURNS VOID AS $$
DECLARE
  v_old_previous_id TEXT;
  v_old_active_id   TEXT;
BEGIN
  SELECT key_id INTO v_old_previous_id
  FROM hmac_keys
  WHERE status = 'PREVIOUS';

  SELECT key_id INTO v_old_active_id
  FROM hmac_keys
  WHERE status = 'ACTIVE';

  IF v_old_previous_id IS NOT NULL THEN
    UPDATE hmac_keys
    SET status = 'RETIRED', retired_at = now()
    WHERE key_id = v_old_previous_id;
  END IF;

  IF v_old_active_id IS NOT NULL THEN
    UPDATE hmac_keys
    SET status = 'PREVIOUS', rotated_at = now()
    WHERE key_id = v_old_active_id;
  END IF;
