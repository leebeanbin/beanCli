...
const CompactView: React.FC<CompactProps> = ({ messages, aiStatus, model }) => {
  ...
  return (
    <Box flexDirection="column" flexGrow={1}>
      ...
      {messages.length === 0 ? (
        <Text color="#374151" dimColor>Ask anything...</Text>
      ) : (
        last && (
      ...
      <Text color="#1a2a3a">{'─'.repeat(20)}</Text>
      <Text color="#374151" dimColor>Tab/4: open chat</Text>
    </Box>
  );
};
...
const FullView: React.FC<FullViewProps> = ({
  messages, input, streaming, spinIdx, aiStatus, model, panelWidth, onSqlExec: _onSqlExec,
}) => {
  ...
  return (
    <Box flexDirection="column" flexGrow={1}>
      ...
      {messages.length === 0 && (
        <Box flexDirection="column">
          <Text color="#374151" dimColor>Ask about your database.</Text>
          <Text color="#374151" dimColor>{'/sql <query>  /clear  /model'}</Text>
        </Box>
      )}
        ...
        <Box key={i} flexDirection="column" marginBottom={0}>
          ...
          {msg.sql && (
            <Box flexDirection="column" marginTop={0}>
              ...
              <Text color="#374151" dimColor>Enter:execute</Text>
            </Box>
          )}
          <Text color="#1a2a3a">{'─'.repeat(Math.min(msgW, 30))}</Text>
        </Box>
      ...
      <Text color="#374151" dimColor>Enter:send  Esc:clear</Text>
    </Box>
  );
};
...
export const AiPanel: React.FC = () => {
  const {
    ...
    connectionService,
    setPendingSql, setAppMode, setFocusedPanel,
  } = useAppContext();
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    // Slash commands
    if (text.startsWith('/clear')) { setMessages([]); setInput(''); return; }
    if (text.startsWith('/model ')) { setModel(text.slice(7).trim()); setInput(''); return; }
    if (text.startsWith('/sql ')) {
      const sql = text.slice(5).trim();
      if (sql) { setPendingSql(sql); setAppMode('query'); setFocusedPanel('query'); }
      setInput('');
      return;
    }
    ...
    if (!connectionService?.streamAi) {
      setMessages(prev => [...prev, {
        role: 'assistant', content: 'AI not available. Check that the beanllm sidecar is running.',
        streaming: false, ts: new Date(),
      }]);
      setInput('');
      return;
    }
    ...
    // Add user message
    const userMsg: ChatMessage = { role: 'user', content: text, streaming: false, ts: new Date() };
    ...
    setMessages(prev => [...prev, {
      role: 'assistant', content: '', streaming: true, ts: new Date(), model,
    }]);

    await connectionService.streamAi(history, { model }, {
      onChunk: (chunk) => {
        ...
        setMessages(prev => {
          ...
          if (last?.streaming) {
            updated[updated.length - 1] = {
              ...last, content, sql, model: mdl || model, streaming: false,
            };
          }
          return updated;
        });
  ...
  useInput((inp, key) => {
    ...
    if (inp && inp >= ' ' && !key.ctrl && !key.meta) { setInput(s => s + inp); return; }
  });
