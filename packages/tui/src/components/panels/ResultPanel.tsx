...
export const ResultPanel: React.FC = () => {
  ...
  const viewStart = Math.max(
    0,
    Math.min(cursor - Math.floor(VISIBLE_ROWS / 2), filteredRows.length - VISIBLE_ROWS),
  );
  ...
  if (!queryResult) {
    return (
      <Box flexDirection="column" flexGrow={1}>
        ...
        <Text color="#374151" dimColor>
          Run a query in the editor above
        </Text>
        <Text color="#374151" dimColor>
          or select a table from Schema panel
        </Text>
      </Box>
    );
  }
  ...
  return (
    <Box flexDirection="column" flexGrow={1}>
      ...
      <Box gap={2}>
        ...
        {queryResult.message && <Text color="#10b981">{queryResult.message}</Text>}
      </Box>
