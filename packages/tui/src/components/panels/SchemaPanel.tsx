      ...
      {connections.map((conn, i) => {
        ...
        return (
          <Box key={conn.id}>
            ...
            <Text
              color={isCursor ? '#0a1628' : typeColor}
              backgroundColor={isCursor ? '#00d4ff' : undefined}
            >
              {` ${conn.type.slice(0, 4)}`}
            </Text>
  ...
  useInput((input, key) => {
    ...
    if (phase === 'list' || phase === 'error') {
      ...
      if (key.downArrow || input === 'j') {
        setListCursor((c) => Math.min(maxList, c + 1));
        return;
      }
    ...
    if (phase === 'connected') {
      ...
      if (key.downArrow || input === 'j') {
        setTableCursor((c) => Math.min(maxTable, c + 1));
        return;
      }
  ...
  const renderBody = () => {
    switch (phase) {
      ...
      case 'error':
        return (
          <Box flexDirection="column" gap={0}>
            ...
            <Text color="#374151" dimColor>
              {connections.length > 0 ? 'n:new e:edit d:del' : 'n:new'}
            </Text>
            <Text color="#374151" dimColor>
      ...
      case 'connecting':
        return (
          <Box flexDirection="column">
            <Text color="#00d4ff">{SPINNER[spinIdx]} Connecting...</Text>
            <Text color="#4a5568" dimColor>
      ...
      case 'connected':
        return (
          <Box flexDirection="column">
            ...
            <Text color="#1a2a3a">{'─'.repeat(20)}</Text>
            <TableTree tables={tables} cursor={tableCursor} isActive={isActive} />
            <Text color="#1a2a3a">{'─'.repeat(20)}</Text>
            <Text color="#374151" dimColor>
              j/k:move Enter:use
            </Text>
            <Text color="#374151" dimColor>
              c:create table D:disc
            </Text>
          </Box>
        );
    }
