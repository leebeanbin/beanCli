...
interface EditOverlayProps {
  ...
  columns:  string[];
  pk:       string;
  overlay:  { type: 'edit'; row: Record<string, unknown>; fieldIdx: number; buffer: string }
          | { type: 'insert'; values: Record<string, string>; fieldIdx: number };
  feedback: string | null;
}

const EditOverlayView: React.FC<EditOverlayProps> = ({ type, columns, pk, overlay, feedback }) => {
  ...
  return (
    <Box
      ...
      paddingX={1}
    >
      <Text color="#00d4ff" bold>{type === 'edit' ? '  EDIT ROW  ' : '  INSERT ROW  '}</Text>
      ...
      {feedback ? (
        ...
        <Text color="#374151" dimColor>Tab:field  Enter:save  Esc:cancel</Text>
      )}
    </Box>
  );
};
...
const DetailView: React.FC<{
  ...
  rowIdx: number; rowCount: number; panelWidth: number;
}> = ({ tableName, columns, row, rowIdx, rowCount, panelWidth }) => (
  <Box flexDirection="column" flexGrow={1}>
    <Box gap={2}>
      <Text color="#00d4ff" bold>{tableName}</Text>
      <Text color="#4a5568">Row {rowIdx + 1} / {rowCount}</Text>
    </Box>
    ...
    <Text color="#1a2a3a">{'─'.repeat(Math.min(panelWidth, 80))}</Text>
    <Text color="#374151" dimColor>j/k:row  v:table  e:edit  d:delete  [/]:cycle  r:reload  Q:query</Text>
  </Box>
);
...
export const ExplorePanel: React.FC = () => {
  const {
    focusedPanel, overlay: appOverlay, paletteOpen,
    browseTable, setBrowseTable, tables,
    connectionService, activeConnection,
    setAppMode, setFocusedPanel, setPendingSql,
    userRole,
  } = useAppContext();
  ...
  const [schema,     setSchema]     = useState<Array<{column: string; type: string; nullable: string; default: string}>>([]);
  const [schemaLoading, setSchemaLoading] = useState(false);

  const isActive =
    (focusedPanel === 'query' || focusedPanel === 'result') &&
    !appOverlay && !paletteOpen;
  ...
  const fetchSchema = useCallback(async (tableName: string) => {
    ...
    const schemaFilter = dbType === 'mysql'
    ...
    if (!res.error) {
      setSchema(res.rows as Array<{column: string; type: string; nullable: string; default: string}>);
    }
  }, [connectionService, activeConnection]);

  // Table cycling
  const switchTable = useCallback((dir: 1 | -1) => {
  ...
  const runSql = useCallback(async (sql: string, onSuccess: () => void) => {
    if (!connectionService) return;
    const res = await connectionService.executeQuery(sql);
    if (res.error) {
  ...
  const commitEdit = useCallback(async (
    ov: { type: 'edit'; row: Record<string, unknown>; fieldIdx: number; buffer: string },
  ) => {
    ...
    await runSql(sql, () => { setEditOvl(null); void fetchRef.current(browseTable); });
  }, [editableCols, pk, browseTable, runSql, activeConnection]);

  const commitInsert = useCallback(async (
    ov: { type: 'insert'; values: Record<string, string>; fieldIdx: number },
  ) => {
    if (!browseTable) return;
    const cols = editableCols.filter(c => ov.values[c]?.trim());
    if (cols.length === 0) { setEditOvl(null); return; }
    const dbType = activeConnection?.type;
  ...
  const commitDelete = useCallback(async (pkValue: string) => {
    if (!browseTable) return;
    const dbType = activeConnection?.type;
  ...
  useInput((inp, key) => {
    ...
    if (editOvl) {
      if (editOvl.type === 'delete') {
        if (inp === 'y' || inp === 'Y') { void commitDelete(editOvl.pkValue); return; }
        setEditOvl(null);
        return;
      }

      if (editOvl.type === 'edit') {
        if (key.escape) { setEditOvl(null); return; }
        if (key.return) { void commitEdit(editOvl); return; }
        if (key.tab) {
        ...
        if (key.backspace) { setEditOvl({ ...editOvl, buffer: editOvl.buffer.slice(0, -1) }); return; }
        if (inp && inp >= ' ' && !key.ctrl) { setEditOvl({ ...editOvl, buffer: editOvl.buffer + inp }); return; }
        return;
      }
      ...
      if (editOvl.type === 'insert') {
        ...
        if (key.escape) { setEditOvl(null); return; }
        if (key.return) { void commitInsert(editOvl); return; }
        if (key.tab) {
          const next = (editOvl.fieldIdx + 1) % editableCols.length;
    ...
    if (key.pageUp    || inp === 'u') { setCursor(c => Math.max(0, c - PAGE_SIZE)); return; }
    if (key.pageDown  || inp === 'd') { setCursor(c => Math.min(maxCursor, c + PAGE_SIZE)); return; }
    ...
    // Table cycling
    if (inp === '[' || inp === '<') { switchTable(-1); return; }
    if (inp === ']' || inp === '>') { switchTable(1);  return; }
    if (key.tab)                    { switchTable(1);  return; }
    ...
    if (inp === 'e' && filteredRows.length > 0) {
      if (!canWrite) { setFeedback('! Read-only (ANALYST role)'); return; }
      const row  = filteredRows[cursor]!;
      const col  = editableCols[colCursor] ?? editableCols[0] ?? columns[0] ?? '';
      setEditOvl({
        type: 'edit', row, fieldIdx: editableCols.indexOf(col), buffer: String(row[col] ?? ''),
      });
      return;
    }
    ...
    if (inp === 'D' && filteredRows.length > 0) {
      if (!canDelete) { setFeedback('! DBA role required for DELETE'); return; }
      const row     = filteredRows[cursor]!;
    ...
    if (inp === 'Q') {
      if (browseTable) setPendingSql(`SELECT * FROM ${quoteIdent(browseTable, activeConnection?.type)} LIMIT 50;`);
      setAppMode('query');
  ...
  if (!browseTable) {
    return (
      <Box flexDirection="column" flexGrow={1}>
        ...
        <Text color="#4a5568" dimColor>Select a table from Schema panel → Enter</Text>
      </Box>
    );
  }
  ...
  if (loading) {
    return (
      <Box flexDirection="column" flexGrow={1}>
        <Text color="#00d4ff">{SPINNER[spinIdx]} Loading {browseTable}...</Text>
      </Box>
    );
  }
  ...
  if (error) {
    return (
      <Box flexDirection="column" flexGrow={1} minWidth={0}>
        <Text color="#ef4444" bold>✕ {browseTable}</Text>
        <Box minWidth={0}>
          <Text color="#fca5a5" wrap="wrap">{error}</Text>
        </Box>
        <Text color="#374151" dimColor>r:retry  Q:query mode</Text>
      </Box>
    );
  }
  ...
  if (editOvl?.type === 'delete') {
    return (
      <Box flexDirection="column" flexGrow={1}>
        <Text color="#ef4444" bold>DELETE ROW</Text>
        <Text color="#1a2a3a">{'─'.repeat(30)}</Text>
        <Text color="#fca5a5">{pk}: {editOvl.pkValue.slice(0, 30)}</Text>
        <Text color="#1a2a3a">{'─'.repeat(30)}</Text>
        <Text color="#e0e0e0">Confirm delete? <Text color="#10b981" bold>y</Text>/<Text color="#ef4444" bold>n</Text></Text>
      </Box>
    );
  }
  ...
  if (view === 'schema') {
    return (
      <Box flexDirection="column" flexGrow={1}>
        <Box gap={2}>
          <Text color="#00d4ff" bold>{browseTable}</Text>
        ...
        {/* Column header */}
        <Box>
          <Text color="#00d4ff" bold>{'COLUMN'.padEnd(26)}</Text>
          <Text color="#00d4ff" bold>{'TYPE'.padEnd(22)}</Text>
          <Text color="#00d4ff" bold>{'NULL'.padEnd(6)}</Text>
          <Text color="#00d4ff" bold>DEFAULT</Text>
        </Box>
        <Text color="#1a2a3a">{'─'.repeat(Math.min(panelWidth, 80))}</Text>
        ...
        {schema.length === 0 && !schemaLoading && (
          <Text color="#4a5568" dimColor>No schema info found for this table.</Text>
        )}

        <Text color="#1a2a3a">{'─'.repeat(Math.min(panelWidth, 80))}</Text>
        <Text color="#374151" dimColor>s:back to table  [/]:cycle table  r:reload schema</Text>
      </Box>
    );
  }
  ...
  return (
    <Box flexDirection="column" flexGrow={1}>
      ...
      {!filtering && filter && (
        <Box><Text color="#f59e0b">  filter: </Text><Text color="#e0e0e0">{filter}</Text><Text color="#4a5568"> Esc:clear</Text></Box>
      )}
      ...
      <Text color="#374151" dimColor>
        ...
        {canWrite ? <Text color="#374151" dimColor>{'  e:edit  i:ins'}</Text> : null}
        {canWrite ? <Text color="#374151" dimColor>{canDelete ? '  D:del' : ''}</Text> : null}
        {userRole ? <Text color="#4a5568">{'  · '}{userRole}</Text> : null}
      </Text>
    </Box>
  );
};
