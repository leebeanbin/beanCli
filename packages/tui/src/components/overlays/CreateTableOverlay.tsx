  ...
  useInput((inp, key) => {
    ...
    if (area === 'tablename') {
      ...
      if (key.backspace || key.delete) {
        setTableName((s) => s.slice(0, -1));
        return;
      }
      if (inp && inp >= ' ' && !key.ctrl && !key.meta) {
        setTableName((s) => s + inp);
        return;
      }
      return;
    }
    ...
    if (key.downArrow) {
      setRowIdx((r) => Math.min(cols.length - 1, r + 1));
      return;
    }
    ...
    if (colField === 'name') {
      ...
      if (key.backspace || key.delete) {
        setCols((prev) =>
          prev.map((c, i) => (i === rowIdx ? { ...c, name: c.name.slice(0, -1) } : c)),
        );
        return;
      }
      if (inp && inp >= ' ' && !key.ctrl && !key.meta) {
        setCols((prev) => prev.map((c, i) => (i === rowIdx ? { ...c, name: c.name + inp } : c)));
        return;
      }
    }
    ...
    <Box flexGrow={1} flexDirection="column" paddingX={1} paddingY={1}>
      ...
      {cols.map((col, i) => {
        ...
        return (
          <Box key={i}>
            {/* Row number */}
            <Text color={isRow ? '#00d4ff' : '#374151'}>{`  ${String(i + 1).padStart(2)} `}</Text>
      ...
      <Box flexDirection="column" marginTop={0}>
        ...
        {ddl.split('\n').map((line, i) => (
          <Text key={i} color="#4a5568" dimColor>
            {line}
          </Text>
        ))}
      </Box>
